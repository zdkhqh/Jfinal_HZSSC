...
(function ($) {
	...
	*/
	...
	// 这是日历头部
     ...
     $(target).find('.calendar-title span').html(yDisplay + '农历 ' + cyclical(opts.year - 1900 + 36) + '年 【' + Animals[(opts.year - 4) % 12] + '年】')	 
				...
				if (opts.month == day[1]) {
					...
					if (opts.lunarDay) {
						...
						if (opts.solarTerms && info.solarTerms) {
							dayHtml += '<span color="#0080ca">' + info.solarTerms + '</span>';
						} else {
				...
				day.hover(function (e) {
					...
					if (inf) {
						var ct = '<font color="#ffffff" style="font-size:9pt;">' + inf.sYear
							 ...
							 + '<br><font color="white">农历 ' + monthName[inf.lMonth - 1] + '  ' + cDay(inf.lDay, inf.lMonth,true)
	...
	//	alert(today);
		...
		//	alert(current);
			t.find('td[abbr="' + current + '"]').addClass('calendar-selected').css('backgroundColor', '#2997c8');
		}
	...
	$('head').append('<style>.fullcalendar-day {border-left: 1px solid #eee;border-top: 1px solid #eee;} '
		 ...
		 + '.fullcalendar-detail{border-radius:5px;position: absolute; z-index: 10; background-color: #0080ca; display: none; opacity: 0.8; padding: 5px; text-align: right;filter: Alpha(opacity=80);width:150px;}'
